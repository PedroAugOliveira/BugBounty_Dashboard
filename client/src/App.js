...
function App() {
  ...
  // Acunetix Integration States
  const [showAcunetixConfigModal, setShowAcunetixConfigModal] = useState(false);
  const [showAcunetixTargetSelectorModal, setShowAcunetixTargetSelectorModal] = useState(false);
  const [showAcunetixDashboardModal, setShowAcunetixDashboardModal] = useState(false);
  const [isAcunetixImporting, setIsAcunetixImporting] = useState(false);
  const [acunetixConfig, setAcunetixConfig] = useState(null);
  ...
  // Acunetix Modal Handlers
  const handleCloseAcunetixConfigModal = () => setShowAcunetixConfigModal(false);
  const handleOpenAcunetixConfigModal = () => setShowAcunetixConfigModal(true);

  const handleCloseAcunetixTargetSelectorModal = () => setShowAcunetixTargetSelectorModal(false);
  const handleOpenAcunetixTargetSelectorModal = () => setShowAcunetixTargetSelectorModal(true);
  ...
  const startAcunetixBulkImport = async () => {
    if (!mostRecentHttpxScan || !mostRecentHttpxScan.result) {
      alert('No HTTPX results available to import.');
      return;
    }

    setIsAcunetixImporting(true);
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}:${process.env.REACT_APP_SERVER_PORT}/acunetix/bulk-import`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            httpxResults: mostRecentHttpxScan.result,
      ...
      if (response.ok) {
        ...
        setToastTitle('Success');
        setToastMessage(`Successfully imported ${result.importedCount} targets to Acunetix`);
        setShowToast(true);
      } else {
        throw new Error('Failed to import targets');
      }
    } catch (error) {
      console.error('Error importing to Acunetix:', error);
      setToastTitle('Error');
      setToastMessage('Failed to import targets to Acunetix. Check your configuration.');
      setShowToast(true);
    } finally {
      setIsAcunetixImporting(false);
    }
  };

  const handleExportWildcardData = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SERVER_PROTOCOL}://${process.env.REACT_APP_SERVER_IP}:${process.env.REACT_APP_SERVER_PORT}/wildcard/export`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `wildcard_data_${activeTarget?.scope_target}_${new Date().toISOString().split('T')[0]}.json`;
        a.click();
      ...
      console.error('Error exporting wildcard data:', error);
      setToastTitle('Error');
      setToastMessage('Failed to export wildcard data.');
      setShowToast(true);
    }
  };
  ...
  return (
    <Container data-bs-theme="dark" className="App" style={{ padding: '20px' }}>
                      ...
                      <Card.Body className="d-flex flex-column justify-content-between">
                        <div>
                          ...
                          <Card.Text className="text-white small fst-italic text-center mb-3">
                            Comprehensive vulnerability scanning on discovered live web servers and subdomains using community-driven Nuclei templates.
                          </Card.Text>
                          <div className="row text-center mb-3">
                            <div className="col">
                              <Card.Text className="text-white small">
                                <strong>Last Scan:</strong> {mostRecentNucleiScan ? new Date(mostRecentNucleiScan.created_at).toLocaleDateString() : "Never"}
                              </Card.Text>
                              <Card.Text className="text-white small">
                                <strong>Status:</strong> {mostRecentNucleiScanStatus || "N/A"}
                              </Card.Text>
                            </div>
                            <div className="col">
                              <Card.Text className="text-white small">
                                <strong>Total Findings:</strong> <span className="text-danger">{getNucleiTotalFindings() || "0"}</span>
                              </Card.Text>
                              <Card.Text className="text-white small">
                                <strong>Critical Findings:</strong> <span className="text-danger">{getNucleiImpactfulFindings() || "0"}</span>
                              </Card.Text>
                            </div>
                            <div className="col">
                              <Card.Text className="text-white small">
                                <strong>Targets:</strong> {getNucleiSelectedTargetsCount() || "0"}
                              </Card.Text>
                              <Card.Text className="text-white small">
                                <strong>Execution Time:</strong> {mostRecentNucleiScan?.execution_time || "N/A"}
                              </Card.Text>
                            </div>
                          </div>
                        </div>
                        <div className="d-flex justify-content-between w-100 mt-3 gap-2">
                          <Button variant="outline-danger" className="flex-fill" onClick={handleOpenNucleiConfigModal}>Configuration</Button>
                          <Button variant="outline-danger" className="flex-fill" onClick={handleOpenNucleiHistoryModal}>Scan History</Button>
                          <Button variant="outline-danger" className="flex-fill" onClick={handleOpenNucleiResultsModal}>View Results</Button>
                          <Button
                            variant="outline-danger"
                            className="flex-fill"
                            onClick={startNucleiScan}
                            disabled={isNucleiScanDisabled() || isNucleiScanning || mostRecentNucleiScanStatus === "pending" || mostRecentNucleiScanStatus === "running"}
                          >
                            <div className="btn-content">
                              {isNucleiScanning || mostRecentNucleiScanStatus === "pending" || mostRecentNucleiScanStatus === "running" ? (
                                <div className="spinner"></div>
                              ) : 'Start Nuclei Scan'}
                            </div>
                          </Button>
      ...
      {/* Acunetix Integration Modals */}
      <AcunetixConfigModal
        show={showAcunetixConfigModal}
        handleClose={handleCloseAcunetixConfigModal}
        onSaveConfig={(config) => setAcunetixConfig(config)}
      />

      <AcunetixTargetSelectorModal
        show={showAcunetixTargetSelectorModal}
        handleClose={handleCloseAcunetixTargetSelectorModal}
        activeTarget={activeTarget}
        httpxResults={mostRecentHttpxScan}
        onImportTargets={(result) => {
          setToastTitle('Success');
          setToastMessage(`Successfully imported ${result.importedCount} targets to Acunetix`);
          setShowToast(true);
        }}
